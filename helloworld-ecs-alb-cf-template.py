...
t.add_resource(alb.LoadBalancer(
    "LoadBalancer",
    Scheme="internet-facing",
    Subnets=Split(',', ImportValue(Join("-", [Select(0, Split("-", Ref("AWS::StackName"))), "cluster-public-subnets"]))),
    SecurityGroups=[Ref("LoadBalancerSecurityGroup")]
))

t.add_resource(alb.TargetGroup(
    "TargetGroup",
...
t.add_resource(alb.Listener(
    "Listener",
    Port="3000",
    Protocol="HTTP",
    LoadBalancerArn=Ref("LoadBalancer"),
    DefaultActions=[
       alb.Action(Type="forward", TargetGroupArn=Ref("TargetGroup"))
    ]
))
